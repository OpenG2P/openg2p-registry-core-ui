...
const StatsCardSmall = ({
  ...
  const { title, rows } = useMemo(() => {
    ...
    if (stats_endpoint.includes('incoming')) {
      return {
        ...
        rows: [
          { id: 'partners', label: 'Partners', value: data.partners, imageUrl: data.imageUrl },
          { id: 'models', label: 'Data Models', value: data.data_models, imageUrl: data.imageUrl },
        ],
      };
    }
    ...
    if (stats_endpoint.includes('outgoing')) {
      return {
        ...
        rows: [
          { id: 'topics', label: 'Topics', value: data.topics, imageUrl: data.imageUrl },
          { id: 'models', label: 'Data Models', value: data.data_models, imageUrl: data.imageUrl },
        ],
      };
    }
  ...
  return (
    <div
      className={`
        flex flex-col justify-between border-4 transition-all duration-200
        w-full h-auto rounded-[44px] px-6 py-5
        ${active ? 'border-black bg-black text-white' : 'border-gray-200 bg-[#FFFFFF] text-gray-900'}
      `}
    >
